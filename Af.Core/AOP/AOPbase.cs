...
namespace Af.Core.AOP
{
    public abstract class AOPbase : IInterceptor
    {
        ...
        protected string CustomCacheKey(IInvocation invocation)
        {
            ...
            var methodArguments = invocation.Arguments.Select(GetArgumentValue).Take(5).ToList();//获取参数列表，最多五个
        ...
        protected static string GetArgumentValue(object arg)
        {
            ...
            if (arg != null)
            {
                ...
                if (arg.GetType().IsClass && arg.GetType().IsSerializable)
                {
        ...
        {
            SqlSugar.ExpressionContext expContext = new SqlSugar.ExpressionContext();
            expContext.Resolve(expression, SqlSugar.ResolveExpressType.WhereSingle);
